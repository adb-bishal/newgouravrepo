...
class BatchClassViewController extends GetxController {
  ...
  //       });
  // }
  Future<void> onClassSearch(query) async {
    if (query.isEmpty) {
  ...
  void performSearch(String query) {
    ...
    final results = dataPagingController.value.list.where((item) {
      ...
      final title = item.title?.toLowerCase() ??
          ''; // If title is null, use an empty string
    ...
    await liveProvider.getLiveData(
        ...
        onSuccess: (message, json) {
          ...
          for (var item in newItems) {
            ...
            isTrialList
                .add(RxBool(isTrial)); // Dynamically add new RxBool items
          }
